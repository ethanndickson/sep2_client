...
A naive implementation that addresses this would have each individual background task (one for each polled resource) wake intermittently, and check if it's time to poll their resource. This would be an unnecessary overhead of tasks repeatedly waking and sleeping.

Our final implementation uses a poll task queue, implemented using a binary heap that sorts tasks by their next scheduled poll time. Then, we use a single task that sleeps, and wakes intermittently to check if it is time to poll the resource at the top of the queue. Once that time arrives, the resource is polled and the stored user call-back is called with the result. The next time to poll on the poll task is then updated before pushing it to the back of the queue. This process is entirely asynchronous, if it is time to poll multiple resources, each request will be sent off without waiting for the previous to complete. 
